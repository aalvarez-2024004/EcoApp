...
export default function CommentSection({ postId, currentUserId, currentUser, onToast, onCommentCountChange }) {
    ...
    const loadComments = useCallback(async () => {
        ...
        try {
            ...
            setComments(list)
            // Notificar al PostCard cuántos comentarios hay
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, maxHeight: 320, overflowY: 'auto', paddingRight: 4 }}>
                {loadingComments ? (
                    // Mini skeletons para comentarios
                    <>
                        {[1, 2].map(i => (
                            <div key={i} style={{ display: 'flex', gap: 10, padding: '10px 14px', background: '#F1F7E8', borderRadius: 16 }}>
                                ...
                                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 6, paddingTop: 2 }}>
                                    <div style={{ height: 10, width: '35%', borderRadius: 5, background: '#D6EABC', animation: 'shimmer 1.4s infinite linear', backgroundImage: 'linear-gradient(90deg,#D6EABC 25%,#C0DD97 50%,#D6EABC 75%)', backgroundSize: '400px 100%' }} />
                                    <div style={{ height: 9, width: '80%', borderRadius: 5, background: '#D6EABC', animation: 'shimmer 1.4s infinite linear', backgroundImage: 'linear-gradient(90deg,#D6EABC 25%,#C0DD97 50%,#D6EABC 75%)', backgroundSize: '400px 100%' }} />
                                </div>
                            </div>
                        ))}
                    </>
